...
def show_dpsroi_offset(im, boxes, offset, classes, trans_std=0.1, save_name=''):
    plt.cla
...
def show_dconv_offset(im, all_offset, step=[2, 2], filter_size=3,
                      dilation=2, pad=2, plot_area=2, plot_level=3, save_name='dconv_offset'):
...
def mshow_dconv_offset(im, all_offset, step=[2, 2], filter_size=3,
                       dilation=2, pad=2, plot_area=2, plot_level=3, save_name='dconv_offset'):
    vis_attr = {'filter_size': filter_size, 'dilation': dilation, 'pad': pad,
                'plot_area'  : plot_area, 'plot_level': plot_level}
    ...
    start_h = np.round(step_h / 2)
    start_w = np.round(step_w / 2)
    start_w, start_h, nw, nh = 20, 18, 0, 0
    stop_w, stop_h = start_w + nw * step_w, start_h + nh * step_h
    plt.figure()
    for im_h in range(start_h, stop_h + 1, step_h):
        for im_w in range(start_w, stop_w + 1, step_w):
            target_point = np.array([im_h, im_w])
            source_y = np.round(target_point[0] * im.shape[0] / map_h)
            source_x = np.round(target_point[1] * im.shape[1] / map_w)
            if source_y < plot_area or source_x < plot_area \
            ...
            save_path = '/home/chenjunjie/workspace/GuidedOffset/logs/figures/' + \
                        save_name + '_' + str(im_w) + '_' + str(im_h) + '.jpg'
            print save_path
            plt.savefig(save_path)


def mshow_dpsroi_offset(im, boxes, offset, classes, trans_std=0.1, save_name=''):
    plt.cla
    for idx, bbox in enumerate(boxes):
        plt.figure(idx + 1)
        plt.axis("off")
